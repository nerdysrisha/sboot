#Simple commands to start. Ensure Docker is running
...
#Stop the jar that is running
docker build -t hworld-observability:latest .
docker images
docker run -p 8080:8080 hworld-observability:latest   # Run in foreground
docker run -d -p 8080:8080 hworld-observability:latest # Run detached
docker run -d --name pistaworld -p 8080:8080 hworld-observability:latest #Run detached with container name provided
docker container ls
docker logs -f pistaworld
curl -X GET http://localhost:8080/api/hello
curl -X GET http://localhost:8080/actuator/prometheus     #Check prometheus is scraping etc



#####Detailed Prom and Graf ###############

PS C:\ws\sboot\lab047b> docker start prometheus
prometheus
PS C:\ws\sboot\lab047b> docker start grafana
grafana
PS C:\ws\sboot\lab047b>




# Copy prometheus.yml from container to local machine


docker cp prometheus:/etc/prometheus/prometheus.yml C:\ws\sboot\lab047b\prometheus.yml
Successfully copied 2.56kB to C:\ws\sboot\lab047b\prometheus.yml


# Edit prometheus.yml manually and add scarape configs. Check the file in this directory. 'prometheus.yml

# Copy updated prometheus.yml back to the container



docker cp C:\ws\sboot\lab047b\prometheus.yml prometheus:/etc/prometheus/prometheus.yml
Successfully copied 2.56kB to prometheus:/etc/prometheus/prometheus.yml
